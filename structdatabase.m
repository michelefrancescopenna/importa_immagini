...
for j=1:size(images(i).cartella,1)
    ...
    images(i).cartella(j).thumb=imresize(images(i).cartella(j).foto, [64 64]);
    images(i).cartella(j).doub=im2double(images(i).cartella(j).thumb64);
    images(i).cartella(j).fft=fft2(images(i).cartella(j).doub64);
